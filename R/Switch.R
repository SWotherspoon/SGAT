...
##' @title Cluster Model Structures
...
cluster.model <- function(model,beta=NULL,B0=1,b0=NULL) {
  ...
  with(env, {
    ## Contribution to log posterior from the movement for state b
    ...
    ## Contribution to log posterior for allocatied
    ...
    model$estelle.logpb <- estelle.logpb
    model$stella.logpb <- stella.logpb
    model$estelle.logpB <- estelle.logpB
    model$stella.logpB <- stella.logpB
    model$n.states <- nrow(beta)
  })
  env$model
}
...
##'
##' These functions draw samples form posterior for the simple cluster
##' model of behavioural switching Stella or Estelle model by the
...
##' @export
estelle.metropolis.cluster <- function(model,
...
##' @rdname estelle.metropolis.cluster
##' @export
stella.metropolis.cluster <- function(model,
                                     proposal.x,
                                     x0=NULL,b0=NULL,
                                     iters=1000L,thin=10L,chains=1L,
                                     verbose=interactive()) {
