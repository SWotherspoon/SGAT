...
estelle.metropolis <- function(model,
  ...
  if(is.list(x0))
    x0 <- array(x0,c(dim(x0[[1]])[1:2],chains))
  else
    x0 <- array(x0,c(dim(x0)[1:2],chains))
  if(is.list(z0))
    z0 <- array(unlist(z0),c(dim(x0[[1]])[1:2],chains))
  else
...
chain.collapse <- function(s,collapse=TRUE,discard=0,thin=1,chains=NULL) {
  subset <- function(s) s[,,seq.int(from=1+max(discard,0),to=dm[3L],by=thin)]
  if(!is.list(s)) {
    if(thin>1 || discard>0)
      s <- subset(s)
  } else {
...
##' @rdname chain.coda
##' @export
chain.coda <- function(s,collapse==FALSE,chains=NULL) {
  rate <- function(s) mean(apply(s,1,function(x) mean(rowMeans(x[,-1L]-x[,-ncol(x)]!=0))))
