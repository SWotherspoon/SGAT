...
##' @export
solar.residuals <- function(twilight,rise,p,zenith=96) {
  sgn <- ifelse(rise,1,-1)
  s <- solar(twilight)
...
##' @param chains the set of chains to retain, or \code{NULL}.
##' @export
chain.apply <- function(s1,f,collapse=TRUE,chains=NULL) {
  s1 <- chain.collapse(s1,collapse=collapse,chains=chains)
...
##' @rdname chain.apply
##' @export
chain.apply2 <- function(s1,s2,f,collapse=TRUE,chains=NULL) {
  s1 <- chain.collapse(s1,collapse=collapse,chains=chains)
  s2 <- chain.collapse(s2,collapse=collapse,chains=chains)
