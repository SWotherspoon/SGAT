...
##'
##' If \code{\link{pimg}} is supplied \code{\link{grid}} and
##' \code{\link{proj}} are ignored and binning is added to the
##' existing \code{\link{pimg}}. If \code{\link{pimg}} or
##' \code{\link{grid}} are not supplied a sensible default is
##' created. (If \code{\link{grid}} and \code{\link{proj}} are
##' supplied the proj is ignored, unless projection(grid) is NA).
##'
...
chain.bin <- function(fit, bin = c("primary", "intermediate"),
    ...
    ## case 2: pimg is missing
    if (is.null(pimg)) {
        ## case 3: grid is also missing
        if (is.null(grid)) {
            grid <- .chaingrid(chain)
        } else {
            ## case 4: grid is not missing
            grid <- raster(grid)
        }

        pimg <- Pimage(fit$twilight, grid = grid, Z = switch(bin, primary = FALSE, intermediate = TRUE))
    }
