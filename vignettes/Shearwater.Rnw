...
selection of starting locations as they must then also satisfy this
...
with locations that fall on the land, but when those location leave
the land they will never return there.
...
lastz <- chain.last(fit$z)
lapply(lastx,function(x) all(fixedx | model$residuals(x) > 0))
lapply(lastx,function(x) all(fixedx | is.sea(x)))
lapply(lastz,function(z) all(is.sea(z)))
@
...
x <- chain.collapse(fit$x)
k <- sample(dim(x)[1],20)
matplot(t(x[k,1,]),type="l",lty=1,col="dodgerblue",ylab="Lon")
matplot(t(x[k,2,]),type="l",lty=1,col="firebrick",ylab="Lat")
par(opar)
@
\begin{figure}[ht]
  \centering
...
for(k in 1:length(fit$z)) {
  fit$z[[k]][fixedz,1,] <- 147.670
  fit$z[[k]][fixedz,2,] <- -43.133
}
@
...
<<label=twl2,eval=F>>=
x <- chain.collapse(fit$x)
plot(t(x[2,,]),pch=16,cex=0.7,col="dodgerblue1",xlab="Lon",ylab="Lat")
plot(wrld_simpl,add=T,col="grey80")
points(colMeans(t(x[2,,])),pch=16,col="red")
@
...
samples through either a two dimensional histogram or kernel density
estimate.  The \texttt{slices} function defines time intervals into
which the track will be split, and the \texttt{slice} function bins
the samples for each interval to form a density estimate. Binning the
$z$ locations weighted by the time intervals between twilights
estimates the distribution of time spent.  Usually an appropriate
weighting is computed by default, but in this case it is necessary to
weight out the fixed $z$ locations to prevent them dominating (Figure~\ref{fig:wbin})
<<label=wbin,eval=F>>=
r <- raster(nrows=180,ncols=160,xmn=80,xmx=170,ymn=-70,ymx=-10)
dt <- ifelse(fixedz,0,diff(as.numeric(model$time)/3600))
s <- slices(type="intermediate",breaks="weeks",mcmc=fit,grid=r,weights=dt)
opar <- par(mfrow=c(3,2),mar=c(3,4,3,2)+0.1)
for(k in slice.indices(s)) {
  tm <- slice.interval(s,k)
  sk <- slice(s,k)
  plot(sk,useRaster=F,main=sprintf("%s - %s",as.Date(tm[1]),as.Date(tm[2])))
  plot(wrld_simpl,add=T,col="grey80")
  plot(elide(wrld_simpl,shift=c(360,0)),add=T,col="grey80")
}
par(opar)
@
\begin{figure}[ht]
  \centering
<<fig=T,echo=F,height=8>>=
<<wbin>>
@
