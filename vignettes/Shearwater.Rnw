...
all(fixedx | model$residuals(chain.last(fit$x[[1]])) > 0)
all(fixedx | is.sea(chain.last(fit$x[[1]])))
all(is.sea(chain.last(fit$z[[1]])))
@
...
fit$z[[1]][fixedz,1,] <- 147.670
fit$z[[1]][fixedz,2,] <- -43.133
@
