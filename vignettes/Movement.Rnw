...
matplot(scale(t(fit$x[[1]][150,,]),scale=F),type="l",lty=1,col=c(2,4),
        xlab="",ylab=expression(x[150]))
matplot(scale(t(fit$z[[1]][150,,]),scale=F),type="l",lty=1,col=c(2,4),
...
\caption{The posterior means (red) and $95\%$ credible intervals
  ...
  \label{fig:LFci}
\end{figure}
Figure~\ref{fig:LFpath} shows the mean twilight locations together with
the samples for a several twilight and several intermediate locations.
<<label=LFpath,eval=F>>=
## Plot sequence of mean x's
mn <- location.mean(fit$x)
x <- chain.collapse(fit$x)
z <- chain.collapse(fit$z)
plot(mn,pch=16,cex=0.4,col="grey80",xlab="Lon",ylab="Lat")
points(t(x[30,,]),pch=16,cex=0.2,col="dodgerblue1")
points(t(x[80,,]),pch=16,cex=0.2,col="dodgerblue1")
points(t(x[200,,]),pch=16,cex=0.2,col="dodgerblue1")
points(t(z[150,,]),pch=16,cex=0.2,col="firebrick1")
points(t(z[100,,]),pch=16,cex=0.2,col="firebrick1")
points(t(z[220,,]),pch=16,cex=0.2,col="firebrick1")
...
\caption{The posterior means (red) and $95\%$ credible intervals
  ...
  \label{fig:HFci}
\end{figure}
Figure~\ref{fig:HFpath} shows the mean twilight locations together with
the samples for a several twilight and several intermediate locations.
<<label=HFpath,eval=F>>=
## Plot sequence of mean x's
mn <- location.mean(fit$x)
